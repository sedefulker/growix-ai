...
const INITIAL_AGENTS: AgentStep[] = [
  { id: 1, label: "Trend Ajanı",   status: "idle" },
  { id: 2, label: "İçerik Ajanı",  status: "idle" },
  { id: 3, label: "Rakip Ajanı",   status: "idle" },
  { id: 4, label: "Fiyat Ajanı",   status: "idle" },
  { id: 5, label: "Kâr Ajanı",     status: "idle" },
];

const PLATFORM_OPTIONS: { value: PlatformKey; label: string }[] = [
  { value: "trendyol",    label: "Trendyol — %15 komisyon"    },
  { value: "hepsiburada", label: "Hepsiburada — %14 komisyon" },
  { value: "n11",         label: "n11 — %12 komisyon"         },
];
...
export default function Home() {
  ...
  const selectFile = useCallback((f: File) => {
    if (!f.type.startsWith("image/")) {
      setError("Geçerli bir görsel dosyası seçin (PNG, JPG, WEBP).");
      return;
    }
    setFile(f);
  ...
  const updateAgent = (agentId: number, status: AgentStatus) => {
    setAgents((prev) =>
  ...
  const onSubmit = async () => {
    ...
    body.append("brief_description", description);
    body.append("cost_price",        costPrice);
    body.append("platform",          platform);

    try {
      const res = await fetch(`${API_BASE}/analysis/stream`, {
        method: "POST",
        body,
      });
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (agent >= 1 && agent <= 5) {
              updateAgent(agent, status as AgentStatus);
            }

            if (status === "complete" && data) {
              // Orkestratör'den gelen tam sonucu map'le
              const content  = data.content  || {};
              const pricing  = data.pricing  || {};
              const profit   = data.profit   || {};
              const scenario = profit.scenarios?.[2] || {};

              setResult({
                id:                data.listing_id,
                seo_title:         content.seo_title         || "",
                base_description:  content.base_description  || "",
                tone_sincere:      content.tone_sincere       || "",
                tone_professional: content.tone_professional  || "",
                tone_youthful:     content.tone_youthful      || "",
                tags:              content.tags               || [],
                suggested_price:   pricing.optimal_price      || 0,
                estimated_profit:  scenario.net_profit        || 0,
                pricing_logic:     content.pricing_logic      || "",
                cost_price:        data.cost_price            || parseFloat(costPrice),
                commission_amount: pricing.commission_amount  || 0,
                commission_rate:   pricing.commission_amount / (pricing.optimal_price || 1),
                cargo_cost:        pricing.cargo_cost         || 0,
                net_profit:        pricing.net_profit_per_unit || 0,
                profit_margin:     pricing.margin_at_optimal  || 0,
                platform:          data.platform              || platform,
              });
              setStep(3);
            }

            if (status === "error" && agent === 0) {
              setError(data?.message || "Analiz sırasında hata oluştu.");
            }
          } catch {
            // JSON parse hatası — satırı geç
          }
        }
      }
    } catch (err: any) {
      setError(err.message || "Bağlantı hatası. Backend çalışıyor mu?");
    } finally {
      setGenerating(false);
    }
  };
  ...
  const activeDesc = useMemo(() => {
    ...
    return tone === "sincere"
      ? result.tone_sincere
      : tone === "professional"
      ? result.tone_professional
      : result.tone_youthful;
  }, [result, tone]);

  const isReady       = !!file && description.trim().length > 0 && costPrice.length > 0;
  const agentsStarted = agents.some((a) => a.status !== "idle");
  ...
  return (
    <>
      <style>{`
        ...
        /* STATS */
        .stats { display: flex; gap: 0; margin-bottom: 40px; }
        .stat  { padding: 0 28px 0 0; margin-right: 28px; border-right: 1px solid var(--border-2); }
        .stat:last-child { border-right: none; margin-right: 0; }
        .stat-n { font-family: var(--font-display); font-size: 36px; font-weight: 600; color: var(--ink); line-height: 1; margin-bottom: 5px; }
        ...
        .step-item { flex: 1; padding: 12px 16px; border-right: 1px solid var(--border); display: flex; align-items: center; gap: 10px; transition: background 0.15s; }
        .step-item:last-child { border-right: none; }
        .step-done   { background: var(--paper-2); }
        .step-active { background: var(--ink); }
        .step-num  { width: 22px; height: 22px; font-family: var(--font-mono); font-size: 10px; font-weight: 500; border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; flex-shrink: 0; color: var(--ink-4); }
        .step-done   .step-num { border-color: var(--red); color: var(--red); }
        .step-active .step-num { border-color: #fff; color: #fff; }
        .step-label  { font-family: var(--font-mono); font-size: 9px; letter-spacing: 0.1em; text-transform: uppercase; color: var(--ink-4); }
        .step-active .step-label { color: #C8C2B0; }

        /* RIGHT / FORM */
        .right { padding-left: 44px; position: sticky; top: 24px; }

        .form-head { display: flex; align-items: baseline; justify-content: space-between; padding-bottom: 14px; border-bottom: 2px solid var(--ink); margin-bottom: 22px; }
        ...
        /* UPLOAD */
        .upload-zone { display: block; border: 1px solid var(--border); background: var(--paper-2); cursor: pointer; margin-bottom: 16px; transition: border-color 0.14s, background 0.14s; }
        ...
        .upload-icon  { width: 38px; height: 38px; border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; margin: 0 auto 12px; }
        ...
        .preview-meta { padding: 8px 12px; background: var(--ink); display: flex; align-items: center; justify-content: space-between; }
        ...
        .preview-change { font-family: var(--font-mono); font-size: 10px; color: var(--red); cursor: pointer; flex-shrink: 0; margin-left: 8px; }
        ...
        /* RESULTS */
        .results { max-width: 1280px; margin: 0 auto; padding: 0 48px 100px; }
        ...
        /* BREAKDOWN */
        .breakdown-strip { display: grid; grid-template-columns: repeat(3, auto) 1fr; border: 1px solid var(--border); border-top: none; background: var(--paper-2); margin-bottom: 20px; }
        ...
        .bc-logic { font-size: 12px; font-weight: 300; font-style: italic; color: var(--ink-3); line-height: 1.65; }
        ...
        /* FOOTER */
        .result-footer { margin-top: 20px; padding: 14px 18px; border: 1px solid var(--border); background: var(--paper-2); display: flex; align-items: center; justify-content: space-between; }
      ...
      <main>
        <div className="home-grid">
          {/* ── LEFT — EDITORIAL ── */}
          <div className="left">
            <p className="eyebrow">Baş Makale · Satıcı İstihbaratı</p>
            ...
            <p className="deck">
              ...
              Growix'in 5 ajanı ürün görselinizi analiz ederek pazar trendini, rekabeti,
              optimum fiyatı ve kâr senaryosunu saniyeler içinde hesaplar.
            </p>
            ...
            <div className="steps">
              {[
                ...
                const s = i + 1;
                const cls = step > s ? "step-done" : step === s ? "step-active" : "";
                return (
          ...
          <div className="right">
            <div className="form-head">
              ...
              <div className="form-sub">5 Ajan · Platform Raporu</div>
            </div>
            ...
            >
              ...
              {preview ? (
                ...
                <div className="upload-inner">
                  ...
                  <p className="upload-p">Görsel yükleyin veya sürükleyin</p>
            ...
            <div className="field-row">
              <div>
                <label className="field-label" htmlFor="cost">Maliyet (₺)</label>
                <input
                  id="cost"
                  type="number"
            ...
            {/* AJAN PROGRESS — sadece analiz başladığında göster */}
            {agentsStarted && (
            ...
            >
              {generating
                ? <><div className="spinner" aria-hidden="true" /> 5 Ajan Çalışıyor</>
                : "— Raporu Oluştur —"}
            </button>

            {error && (
              <div className="err-bar" role="alert">{error}</div>
            )}
          </div>
        </div>
        ...
        {result && (
          <div className="results">
              ...
              <div className="block out-full">
                <div className="block-head">
                  ...
                  <button
                    ...
                    onClick={() => copyText(result.seo_title, "title")}
                  >
              ...
              <div className="block out-full">
                <div className="block-head">
                  ...
                  <button
                    ...
                    onClick={() => copyText(result.tags.join(", "), "tags")}
                  >
            ...
            <div className="result-footer">
              ...
              <Link href="/dashboard" className="result-footer-link">
